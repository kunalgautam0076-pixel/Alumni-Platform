...
router.post('/register', async (req, res) => {
  try {
    ...
    user = new User({ name, email, password: hashed, bio, education, workplace, achievements, approved: false });
    await user.save();

    return res.status(201).json({ message: 'Registration request submitted. Wait for admin approval.' });
  } catch (err) {
    console.error(err);
